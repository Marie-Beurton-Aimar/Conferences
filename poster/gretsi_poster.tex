...
\begin{tikzpicture}
\Put(-14,80){\includegraphics[scale=1.8]{images/labri}}
\Put(-6,80){\includegraphics[scale=1.8]{images/ubordeaux}}
\Put(10,80){\includegraphics[scale=1.8]{images/inra}}
...
%    	  \end{tikzfigure}
  ...
  \blocknode{Manual landmarks}%
  {
  	...
  	\begin{tikzfigure}[The mandibles with manual landmarks]
    	  	\centering
    	  	\includegraphics[scale=.27]{images/mlandmarksline}~~~~~~
    	  	\includegraphics[scale=.27]{images/mglandmarksline}
    	  \end{tikzfigure}
 ...
 \blocknode%
  ...
  {   
  	Two lists of contours points from segmentation step are registered 
  	by applying Principal Component Analysis\cite{jolliffe2002principal} Iteration (PCAI). 
    \begin{enumerate}
    	\item Compute the centroid point and principal axis of contours.
    	\item Compute the transformation values between two images.
...
\blocknode{SIFT and landmarks}%
  {   
    ...
    \begin{enumerate}
    	...
    	\item Compute the distance between the descriptor of $P_m$ and each $P'_m$. Keep the pixel that have the minimum distance,
    	\item The process stops when all the pixels in $P_s$ are considered.
    \end{enumerate}
  }
