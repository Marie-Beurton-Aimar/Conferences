...
\Put(6,80){\includegraphics[scale=1.8]{images/inra}}
  ...
  {Context}%
  {
	  \begin{itemize}
   		 ...
   		 \item Morphometrics are also used to classify the objects in different groups.
    	  \end{itemize}
%    	  \begin{tikzfigure}
%    	  	\centering
...
%    	  \end{tikzfigure}
  ...
  {
  	\begin{itemize}
  		\item Morphometric landmarks are points of interest in the biological object,
  		\item Landmarks characterize specificities through the shape \\ most often linked to biological information,
  		\item They are usually defined by biologists manually. 
  		\item Images show manual landmarks in beetle mandibles \\ belonging to our sample. How to \textcolor{red}{locate} the landmarks \\ \textcolor{red}{\textbf{automatically}}?
  	\end{itemize}
  	 %\begin{tikzfigure}[]
    	 % 	\centering
  ...
  It includes four steps:
	    \begin{itemize}
    		...
    		\end{itemize}
    		\item \textbf{\underline{Limitation:}} The obtained results from original SIFT method \\ set many landamark candidates.
  ...
  \blocknode{Proposed method}
  {
  	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Scene image
		\end{itemize}		  		
  		\item \textbf{\underline{Output:}} landmarks of scene image
  	...
  	%\begin{tikzfigure}
    	...
    	%  \end{tikzfigure}
    \Put(14.4,11){\includegraphics[scale=.5]{images/method2}}
  }
 ...
 \blocknode%
  ...
  {   Model and scene image are segmented to extract the contours points. The contours points are registered by applying Principal Component Analysis\cite{jolliffe2002principal, shlens2014tutorial} Iteration (PCAI).
    \begin{enumerate}
    	\item Compute the centroid point and principal axis of each list of contour point,
    	\item Compute the transformation values between two lists of contour points,
    	\item Register two lists of contour points,
...
\blocknode{SIFT and landmarks}%
  {   
    ...
    \end{tikzfigure}
    \begin{enumerate}
    	\item A patch $P_m$ is initialized at each manual landmark of model image (size of $9\times9$),
 ...
 % \startthirdcolumn
  ...
  \blocknode {Results on right mandibles}
  {
  	...
  	\begin{tikzfigure}[]
    	...
    	\includegraphics[scale=.8]{images/md_chartlms}
    \end{tikzfigure}
  }
...
\blocknode {Results on left mandibles}
  ...
  \begin{itemize}
  	...
  	\begin{tikzfigure}[]
    	...
    	\includegraphics[scale=.785]{images/mg_chartlms}
    \end{tikzfigure}
  }
%%%%%%%%%% ------------------------------------------ %%%%%%%%%%
  \blocknode%
