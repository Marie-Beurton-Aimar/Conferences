...
\Put(6,80){\includegraphics[scale=1.8]{images/inra}}
  ...
  {Context}%
  {
	  \begin{itemize}
   		 ...
   		 \item Morphometric characteristics have been used to evaluate the evolution of an organism, by finding new or sharpening the definition of old,
   		 \item Morphometrics are also used to classify the objects to different groups in a family.
    	  \end{itemize}
%    	  \begin{tikzfigure}
%    	  	\centering
...
%    	  \end{tikzfigure}
  ...
  \blocknode{Manual landmarks}%
  {
  	...
  	\begin{tikzfigure}[]
    	  ...
    	  \end{tikzfigure}
    	How to \textcolor{red}{locate} the landmarks \textcolor{red}{\textbf{automatically}}?
  }
  

  %%%%%%%%%% ------------------------------------------ %%%%%%%%%%
  \blocknode{SIFT} %
  { SIFT\cite{lowe2004distinctive} is used to extract distinctive features from the images. 
  It includes four steps:
    ...
    \end{itemize}~\\
    \Put(23,2){\includegraphics[scale=3.8]{images/siftc}}
 ...
 \blocknode%
  {Segmentation}%
  {
    \begin{itemize}
		...
		The threshold value is determined by analysing histogram\cite{leestimating}.
		\item Contours points are extracted by Canny algorithm\cite{canny1986computational}.
  ...
  {   
  	Two lists of contour points from segmentation step are registered 
  	by applying Principal Component Analysis\cite{jolliffe2002principal, shlens2014tutorial} Iteration (PCAI).
    \begin{enumerate}
    	\item Compute the centroid point and principal axis of contours,
    	\item Compute the transformation values between two lists of contour points,
    	\item Register two images,
    	\item Sort the contour points followed y-direction,
    	\item Select a subset of contour points and repeat step 1,
...
\blocknode{SIFT and landmarks}%
  {   
    ...
    \end{tikzfigure}
    \begin{enumerate}
    	\item A patch $P_m$ is initialized at each manual landmark of source image (size of $9\times9$),
    	\item Calculate the SIFT descriptor for $P_m$,
    	\item At the same position in target image, a patch $P_s$ is created (size of $36\times36$),
    	\item For each pixel in $P_s$, a patch $P'_s$ is extracted with the same size of $P_m$,
    	\item Calculate the SIFT descriptor for $P'_s$,
    	\item Compute the distance between the descriptor of $P_m$ and each $P'_m$,
    	\item The process stops when all the pixels in $P_s$ are considered. Keep the pixel that has the minimum distance with $P_m$.
    \end{enumerate}
  }
 ...
 % \startthirdcolumn
  ...
  %%%%%%%%%% ------------------------------------------ %%%%%%%%%%
  \blocknode {Results on left and right mandibles}
  {
  	\begin{tikzfigure}[]
    	\centering
    	\includegraphics[scale=1.5]{images/md_rs}~~
    	\includegraphics[scale=.6]{images/md_chartlms}\\[0.2cm]
    	\includegraphics[scale=1.5]{images/mg_rs}~~
    	\includegraphics[scale=.7]{images/mg_chartlms}
    \end{tikzfigure}
  }
